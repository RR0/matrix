angular
    ...
    function ($log, $q, $rootScope, $http) {
      ...
      function loadLabels(labelsInput) {
        return typeof labelsInput === "object"
          ? loadFile(labelsInput)
          : loadURL(labelsInput);
      }

      function loadMatrixData(matrixData, labelsInput) {
        loadLabels(labelsInput).then(
          messages = > {
          msg = messages;
        var i = 0;
      ...
      return {
        load: function (matrixInput, labelsInput) {
          if (typeof matrixInput === "object") {
            loadFile(matrixInput)
              .then(matrixData = > loadMatrixData(matrixData, labelsInput)
          )
          .
            catch(reason = > $rootScope.$broadcast("dataError", reason)
          )
            ;
          } else {
            loadURL(matrixInput)
              ...
              for (var c in question.choices) {
                if (question.choices.hasOwnProperty(c)) {
                  ...
                  if (choice.value !== false) {
                    var knownPhenomenaProbabilities =
                      choice.knownPhenomenaProbabilities;
  ...
  .controller("MatrixFormController", [
    ...
    "$scope",
    "matrixService",
    function ($log, $scope, matrixService) {
      "use strict";
      $scope.load = function () {
        var matrixInput =
          document.getElementById("matrixFile").files[0] || $scope.matrixURL;
        var labelsInput =
          document.getElementById("labelsFile").files[0] || $scope.labelsURL;
      ...
      function questionChanged() {
        $scope.currentQuestion =
          $scope.questions[$scope.questionsKeys[$scope.questionIndex]];
      ...
      $scope.compute = function (key) {
        ...
        $scope.explanations = matrixService.compute(
          $scope.resultsType === "NonProbable"
        );
      };
    }
